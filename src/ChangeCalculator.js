...
class ChangeCalculator extends Component {
    ...
    calChange(amount) {
        ...
        for (let i = 0; amount > 0 && i < coins.length; i++) {
            ...
            if (value <= amount) {
                ...
                res.push({ value: coins[i].value, total: val, text: this.getUnit(coins[i].value, val) });
    ...
    renderChange() {
        var change = this.state.change.map((item) => {
                        ...
                        <Col xs={12}>
                            <form>
                                <FormGroup>
                                    ...
                                    <FormControl type="text" placeholder="0.00" onChange={this.onChange} value={this.state.value} autoFocus />
                                </FormGroup>
                            </form>
                        </Col>
                    </Row>
